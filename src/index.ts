    ...
    (res: http.IncomingMessage) => {
      ...
      res.on('end', () => {
        ...
        while ((match = body.indexOf(startWord, startIndex)) !== -1) {
          ...
          result.push(
            body.slice(
              findIndex + startWord.length,
              body.indexOf(finishWord, findIndex)
            )
          );
